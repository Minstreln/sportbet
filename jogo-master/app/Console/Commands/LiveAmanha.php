...
use App\User;
use App\Events\LiveAmanhaFutebol;
use Illuminate\Support\Facades\Cache;


class LiveAmanha extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:liveAmanha';
    ...
    public function showAmanha()
    {
        ...
        $leagues = Match::select('league')
            ->where('date', '<=', $data_amanha . ' 23:59:00')
        ...
        foreach ($leagues  as $league) {
            ...
            foreach ($matchs as $match) {
                ...
                if ($count == 0 || count($match->odds) == 0) {
                    ...
                    for ($q = 0; $q < 3; $q++) {
                        $return[$i]['match'][$j]['odds'][$q]['type'] = $odd->type ?? 'NULL';
    ...
    public function showHomeAmanha()
    {
        ...
        return array_merge($this->showAmanhaMain(), $this->showAmanha());
    }
    ...
    {
        //dd($this->showHoje());
        $matchs =  TomorowMatchFlash::where('site_id', env('ID_SITE'))->first();

        if ($matchs) {
            $home = TomorowMatchFlash::find($matchs->id);

            $uptade =   $home->update([
                'dados' => json_encode($this->showHomeAmanha()),
            ]);
            echo "exite";
        } else {
            TomorowMatchFlash::create([
                'dados'     => json_encode($this->showHomeAmanha()),
                'site_id'   => env('ID_SITE')
            ]);
            echo "nada";
        ...
        broadcast(new LiveAmanhaFutebol($this->showHomeAmanha()));
    }
}
