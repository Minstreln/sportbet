...
<template>
  <div class="wrapper">
          ...
          <div class="modal-body box box-primary">
            <div class="login-box-body">
              ...
              <div class="form-group has-feedback">
                <input type="text" class="form-control" v-model="username" placeholder="Login" />
        ...
        <div class="modal-content">
          ...
          <div class="modal-body box box-primary">
            <div class="row">
              ...
              <div class="col-md-12">
                <div class="valor-fechamento-positivo">Quantidade de Bilhetes: {{caixaUser.quantidade }}</div>
                <div
                  class="valor-fechamento-positivo"
                >Apostas no Ponto: {{caixaUser.entradas | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-total-aberto"
                >Apostas Aguardando : {{caixaUser.entradas_abertas | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-total-negativo"
                >Total Prêmios: {{caixaUser.saidas | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-positivo"
                >Adiantamentos: {{caixaUser.lancamentos | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-positivo"
                >Comissões: {{caixaUser.comissoes | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-total-positivo"
                  v-if="caixaUser.total >= 0 "
                >Total: {{caixaUser.total | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-total-negativo"
                  v-if="caixaUser.total < 0 "
                >Total: {{caixaUser.total | formatMoeda() }}</div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body box box-primary">
            ...
            <div class="row">
              <clip-loader :loading="loadingCaixa" :color="color" :size="size"></clip-loader>
              <div class="col-md-12" v-if="Object.values(this.relatorio).length > 0">
                <div class="valor-fechamento-positivo">Quantidade: {{relatorio.quantidade }}</div>
                <div
                  class="valor-fechamento-positivo"
                >Entradas: {{relatorio.entradas | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-total-negativo"
                >Saídas: {{relatorio.saidas | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-positivo"
                >Comissões: {{relatorio.comissaocambista | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-total-positivo"
                  v-if="relatorio.saldo >= 0 "
                >Total: {{relatorio.saldo | formatMoeda() }}</div>
                <div
                  class="valor-fechamento-total-negativo"
                  v-if="relatorio.saldo < 0 "
                >Total: {{relatorio.saldo | formatMoeda() }}</div>
    ...
    <div class="modal fade in" id="modal-bilhete">
      <div class="modal-dialog">
        <div class="modal-content" v-for="palpite in bilhetes" :key="palpite.id">
          ...
          <div class="pre-aposta-resul" v-if="logar">
            <h4>
              OBS esses valores poderam ser alterados em decorência de algo que aconteça antes de começar o jogo. Procure o cambista mais próximo, informando o seguinte código:
              <b>{{cupom_pre_aposta}}</b>. Para validação desta aposta:
            </h4>
          </div>
          <div class="header-print-share">
            ...
            <a v-if="logado" class="share-float" @click="printJogos(palpite.id)">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body box box-primary">
            ...
            <div class="input-group busca-time">
              <input type="text" placeholder="PIN..." v-model="pin" class="form-control pin" />
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">
              <i class="fa fa-ticket"></i> Bilhete
            </h4>
          </div>
          ...
          <div class="modal-body box box-primary">
            <div class="col-md-4">
              ...
              <div class="box box-success">
                <div class="box-cupon" v-for="select in selection" :key="select.id">
                  <h5 class="header-campeonato-cupon">
                    <i class="fa fa-trophy"></i>
                    {{select.league}}
                    <span class="delete-palpite-cupon" @click="removePalpite(select.idOdd)">
                      <i class="fa fa-trash"></i>
                    </span>
                  </h5>
                  <li class="cupon-confronto">{{select.home}} X {{select.away}}</li>
                  ...
                  <li>
                    ...
                    <span class="cupon-right">{{select.cotacao | formatCotacao()}}</span>
                  </li>
                </div>
                ...
                <div class="box-rodape-cupon">
                  <h4 class="retorno-individual" v-if="selection.length > 0">
                    ...
                    <span class="val-retorno-ind">{{ total_cotacao | formatCotacao() }}</span>
                  </h4>
                  <h4>Escolha entre os valores mais apostados</h4>

                  <div class="btn-valor-mobile">
                     <button class="btn-valor btn btn-primary" @click="setValApostado(2)">R$ 2,00</button>
                  <button class="btn-valor btn btn-primary" @click="setValApostado(5)">R$ 5,00</button>
                  <button class="btn-valor btn btn-primary" @click="setValApostado(10)">R$ 10,00</button>
                  <button class="btn-valor btn btn-primary" @click="setValApostado(20)">R$ 20,00</button>
                  <button class="btn-valor btn btn-primary" @click="setValApostado(50)">R$ 50,00</button>
                  </div>
                ...
                <div v-if="limitesUser.comissao_premio > 0">
                  ...
                  <span class="val-retorno-ind">{{ retorno  | formatCotacao() }}</span>
                </h4>
                 <h4 class="retorno-individual" v-if="selection.length > 0">
                  <span class="retorno-ind">
                    <b>Cambista Paga:</b>
                  </span>
                  <span class="val-retorno-ind">{{ retornoCambista   | formatCotacao() }}</span>
                </h4>
                </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="input-apostador form-control" v-model="cliente" />
                  </div>
                  ...
                  <button v-if="!live" class="btn btn-success btn-block btnSendBet" @click="enviarAposta">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">
              <i class="fa fa-star"></i> Regulamento
            </h4>
          </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body box box-primary">
                ...
                <table style="width: 100%;" v-if="!live">
                  <tr>
                    <td align="left" width="45%" > <span class="timeMatch"> {{match.home}} </span></td>
                    <td align="center" width="10%">
                      <span class="score-real-time">
                        <strong>  X  </strong>
                      </span>
                    </td>
                    <td align="right" width="45%"> <span class="timeMatch">{{match.away}} </span></td>
                  </tr>
                  <br>
                </table>
                <br>
                ...
                <div class="real-time" v-if="live">
                  <div class="placar">
                        ...
                        <div class="time-real-time" v-if="match.time < 45 && match.time != 0">
                          1º Tempo {{match.time}}
                          <span class="pisca">'</span>
                        </div>
                        ...
                        <table class="tableInfo">
                          <thead>
                            <tr class="table-header">
                              <th class="left padding-10"><span>{{liga}}</span></th>
                              <th class="cell-soccer">1T</th>
                              <th class="cell-soccer">2T</th>
                              <th class="cell-soccer"><label class="icon corner"></label></th>
                              <th class="cell-soccer"><label class="icon yellow-card"></label></th>
                              <th class="cell-soccer"><label class="icon red-card"></label></th>
                            </tr>
                          </thead>
                          ...
                          <tbody>
                            <tr class="table-row">
                              ...
                              <td class="cell-soccer">{{match.numberOfCornersHome}}</td>
                              <td class="cell-soccer">{{match.numberOfYellowCardsHome}}</td>
                              <td class="cell-soccer">{{match.numberOfRedCardsHome}}</td>
                            </tr>
                            <tr class="table-row">
                              ...
                              <td class="cell-soccer">{{match.numberOfCornersAway}}</td>
                              <td class="cell-soccer">{{match.numberOfYellowCardsAway}}</td>
                              <td class="cell-soccer">{{match.numberOfRedCardsAway}}</td>
                            </tr>
                          </tbody>
                        </table>
            ...
            <clip-loader :loading="loading_odds" :color="color" :size="size"></clip-loader>

            <div class="row" v-for="mercado in mercados" :key="mercado.id">
              <div class="titulo-grupo">{{mercado.name}}</div>
              <div class="row">
                <div class="col-md-12" v-for="odd in mercado.odds" :key="odd.id">
                  <div class="odd-match-plus" v-if="odd.cotacao == 0">
                    ...
                    <span
                      ...
                      :taxa="odd.id"
                      @click="addPalpite(odd.uuid, odd.id, match.sport, match.event_id, odd.group_opp, odd.odd, odd.cotacao, liga, match.date, match.home, match.away, odd.type, odd.cotacaoOriginal)"
                    >
    ...
    <aside class="main-sidebar">
      ...
      <section class="sidebar">
        ...
        <div class="user-panel">
          <img style="max-width: 75%; border-radius: 5px;" src="img/logo.png" alt="golbets" />
          <div class="dados-logado">
            <p v-if="logado">{{ name }}</p>
            <p v-if="logado">S. Simples: {{ caixaUser.saldo_simples | formatMoeda() }}</p>
            <p v-if="logado">S. Casadinha: {{ caixaUser.saldo_casadinha | formatMoeda() }}</p>
          </div>
        </div>
        <!--Form Busca-->
        <div class="sidebar-form">
          <div class="input-group">
            <input type="text" v-model="cupom" class="form-control" placeholder="Conferir bilhete" />
        ...
        <ul class="sidebar-menu tree" data-widget="tree">
          ...
          <li class="header">
            <i class="fa fa-trophy"></i> JOGOS DO DIA
          </li>
          ...
          <li class="header">
            <i class="fa fa-trophy"></i> ESPORTES
          </li>
          <li class="treeview" v-if="vivo">
            <a href="javascript:void(0)" class="sidebar-toggle" @click="loadVivo()">
          ...
          <li class="treeview">
            <a href="javascript:void(0)" class="sidebar-toggle" @click="loadFutebol()">
              <i class="fa fa-soccer-ball-o"></i>
              <span>Futebol</span>
            </a>
          </li>
          ...
          <li class="header">
            <i class="fa fa-trophy"></i> LIGAS PRINCIPAIS
          </li>

          <li v-for="league_main in leagues_main" :key="league_main.id">
            <a href="javascript:void(0)" @click="seachLeague(league_main.league)">
              <i class="fa fa-trophy"></i>
        ...
        <div class="row">
          <div class="col-md-8">
                    ...
                    <tr v-for="bilhete in bilhetesLogado" :key="bilhete.id">
                      ...
                      <td>
                        <button
                          ...
                          @click="viewBilhete(bilhete.id, bilhete.status, bilhete.cupom, bilhete.created_at, bilhete.vendedor, bilhete.cliente, bilhete.total_palpites, bilhete.cotacao, bilhete.valor_apostado, bilhete.retorno_possivel,bilhete.tipo )"
                        >
                          <i class="fa fa-eye"></i>
                        </button>
                      </td>
            ...
            <div v-if="jogosView">
              ...
              <div class="content-jogos" v-for="event in filterLiegues" :key="event.id">
                ...
                <div class="jogo">
                  <div class="row container-lista-jogos" v-for="match in event.match " :key="match.id">
                    <div class="col-lg-1 col-md-1 col-xs-2 data-hora">
                      ...
                      <span v-if='!live'>{{ match.date | formatTime() }}</span>
                    </div>
                    <div class="col-lg-5 col-md-5 col-xs-8 jogos">
                      <table style="width: 100%;">
                        <tr>
                          <td width="50%"><h4 style='margin: 1%; font-size: 1.5rem;'>{{match.home}}</h4></td>
                        </tr>
                        <tr>
                          <td width="50%"><h4 style='margin: 1%; font-size: 1.5rem;'>{{match.away}}</h4></td>
                        </tr>
                      </table>
                    </div>
                    <div class="col-lg-1 col-md-1 col-xs-2 data-hora">
                      ...
                      <span v-else>{{ match.date | formatDateHome() }}</span>
                    </div>

                    <div class="col-lg-5 col-md-5 col-xs-12 btn-apostas">
                      <div class="cotacoes-principais" v-for="odd in match.odds" :key="odd.id">
                        <div class="btn-home" :taxaJogo="match.event_id" :taxa="odd.id" @click="addPalpite(odd.uuid, odd.id, match.sport, match.event_id, odd.group_opp, odd.odd, odd.cotacao, event.league, match.date, match.home, match.away, odd.type, odd.cotacaoOriginal)" v-if="odd.cotacao == 0">
                          <i class="fa fa-lock"></i>
                        </div>
                        <div class="btn-home" :taxaJogo="match.event_id" :taxa="odd.id" @click="addPalpite(odd.uuid, odd.id, match.sport, match.event_id, odd.group_opp, odd.odd, odd.cotacao, event.league, match.date, match.home, match.away, odd.type, odd.cotacaoOriginal)" v-if="odd.cotacao > 0">{{odd.cotacao | formatCotacao() }}</div>
                      </div>
                      <div class="plus-odd" @click="loadOdd(event.league, match, event)">
    ...
    <footer class="main-footer">
      ...
      <a  v-bind:href="`https://wwww.${server.host}`">  {{ server.logo }} </a> Todos os direitos reservados.
    </footer>
  </div>
  <!-- ./wrapper -->
</template>
...
export default {
  ...
  computed: {
    filterLiegues() {
        if(this.search != '') {
          return	this.events.filter( x => x.match.some( g =>
                                    g.confronto.toLowerCase().includes(this.search.toLowerCase())
                                    ))
        }else {
          return this.events.filter(league => {

               return league.league.toLowerCase().includes(this.search.toLowerCase())
            })
        }
  ...
  methods: {
    ...
    loadRealtime() {
      window.Echo.channel("live-futebol-hoje").listen("LiveHojeFutebol", e => {
        if (this.vivo) {
          this.events_hoje = e;
      ...
      window.Echo.channel("load-league-main").listen("LoadLigasMain", data => {
      ...
      window.Echo.channel("refreshmatch")
                .listen("LoadRefreshOdd", data => {
    ...
    loadLimites() {
      ...
      axios.get("/api/list-limites").then(response => {
        ...
        this.max_jogos_bilehte = response.data[0]["quantidade_jogos_max_bilhete"];
        this.mini_jogos_bilhete = response.data[0]["quantidade_jogos_mini_bilhete"];
    ...
    viewBilhete(id) {
      ...
      axios
        ...
        .then(response => {
          this.link = 'whatsapp://send?text='+window.location.href+'api/bilhete/'+id;
    ...
    calculaCotacao() {
      ...
      for (var i = 0; i < this.selection.length; i++) {
        ...
        const valComission = this.retorno *  (this.limitesUser.comissao_premio/100);
    ...
    enviarAposta() {
      ...
      if(this.selection.length == 1 && this.apostado > this.caixaUser.saldo_simples) {
        alert("Verifique o saldo em apostas simples!");
        return;
      }
      if(this.selection.length > 1 && this.apostado > this.caixaUser.saldo_casadinha) {
      ...
      if (this.selection.length > this.max_jogos_bilehte) {
        alert(`Quantidade permitida de jogos é de: (${this.max_jogos_bilehte})`);
        return;
      }
      let rota = "";
      if (localStorage.getItem("token") != null) {
        ...
        .then(response => {
         // $("#modal-cupon").modal("hide");
           ...
           if(cota <= this.total_cotacao) {
             axios.post('/api/send-valid-live',{
                  valor_apostado: this.apostado,
                  retorno_possivel: this.retorno,
                  cliente: this.cliente,
                  total_palpites: this.selection.length,
