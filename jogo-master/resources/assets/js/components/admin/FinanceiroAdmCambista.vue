                    ...
                    <div class="modal-body box box-primary">
                        <div class="row">
                            <div class="col-md-12" v-for="u in user" :key="u.id">
                                ...
                                <div class="valor-fechamento">Lançamentos: {{u.lancamentos | formatMoeda() }} </div>  
                                <div class="valor-fechamento">Comissões: {{u.comissoes | formatMoeda() }} </div>  
                                <div class="valor-fechamento-total-positivo" v-if="u.total >= 0 ">Total: {{u.total| formatMoeda() }} </div> 
                                <div class="valor-fechamento-total-negativo" v-if="u.total < 0 ">Total: {{u.total| formatMoeda() }} </div>                            
              ...
              <!-- /.modal-dialog -->
                ...
                <div class="col-md-3">
                      <div class="form-group">
                        ...
                        <select v-model="idCambista" class="form-control" @change="searchCambista(idCambista)">
                            <option>Todos</option>
                                ...
                                <table class="table table-bordered">
                                    <thead class="tabela-class" >
                                        <tr class="gerente">
                                            ...
                                            <th class="tabela-class">LANÇAMENTOS</th>
                                            <th class="tabela-class">COMISSÕES</th>
                                    ...
                                    <tbody class="tabela-class">
                                        <tr v-for="caixa in caixas" :key="caixa.id" >
                                            ...
                                            <td>{{caixa.entradas | formatMoeda() }}</td>
                                            <td>{{caixa.entradas_abertas  | formatMoeda() }}</td>
                                            <td>{{caixa.saidas  | formatMoeda() }}</td>
                                            <td>{{caixa.lancamentos | formatMoeda() }}</td>
                                            <td>{{caixa.comissoes | formatMoeda() }}</td>
                                            <td class="positivo" v-if="caixa.total >= 0">{{caixa.total  | formatMoeda()}}</td>
                                         ...
                                         <tr class="gerente" v-if="idCambista == 'Todos'">
                                            <td>TOTAL</td>
                                            <td>{{ total_apostas  }}</td>
                                            ...
                                            <td class="negativo">{{total_saidas | formatMoeda()}}</td>
                                             <td>{{total_lancamentos  | formatMoeda()}}</td>
...
export default {
    ...
    methods: {
        ...
        loadCaixa () {
              this.loading = true
              axios.get('/admin/list-caixa-adm-cambista')
                    .then((response)=>{
        ...
        searchCambista(id) {
            ...
            if(id == 'Todos') {
                ...
                axios.get('/admin/list-caixa-adm-cambista/'+id)
                    .then((response)=>{
        ...
        sendFechamendo (id, name) {
            ...
            $('#modal-fechamento').modal('show');
                ...
                axios.get('/admin/list-caixa-cambista/'+id)
                    .then((response)=>{
                        this.user = response.data

                        console.log('estou aqui ', this.user)
