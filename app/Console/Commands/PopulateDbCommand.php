...
class PopulateDbCommand extends Command
{
  ...
  protected function populateMainLeagues()
  {
    ...
    foreach ($sportsId as $key => $sport) {
      ...
      $response = $this->get(env('URL_BASE_BETS_API') . 'betfair/ex/upcoming?token='. env('BETS_API_TOKEN') .'&sport_id=' .$key);
      if (empty($response)) {
        continue;
      }
      ...
      for ($page=1; $page <= $pages; $page++) {
        ...
        $response = $this->get(env('URL_BASE_BETS_API') . 'betfair/ex/upcoming?token='. env('BETS_API_TOKEN') .'&sport_id=' .$key. '&page='.$page);
        $response = json_decode($response);
        if ($response->success == 1 && $response->pager->total > 0) {
          foreach ($response->results as $data) {
            echo "\e[0;36;40m Adicionando Liga {$data->league->name}...! \e[0m\n";
      ...
      $upCommings = $this->get(env('URL_BASE_BETS_API') . 'betfair/ex/upcoming?token='. env('BETS_API_TOKEN') .'&sport_id=' .$key);
      ...
      for ($pageUpCommings=1; $pageUpCommings <= $upCommingsPages; $pageUpCommings++) {
        ...
        $upCommings = $this->get(env('URL_BASE_BETS_API') . 'betfair/ex/upcoming?token='. env('BETS_API_TOKEN') .'&sport_id=' .$key. '&page='.$pageUpCommings);
        $upCommings = json_decode($upCommings);
        ...
        foreach ($upCommings->results as $upComming) {
          ...
          //if ($upComming->time_status != 0) {
          //  var_dump($upComming->time_status);
          //} else {
          //  continue;
          //}
